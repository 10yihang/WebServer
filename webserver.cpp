...
void WebServer::sql_pool() {
    ...
    m_connPool->init("127.0.0.1", m_user, m_passWord, m_databaseName, 3306,
...
void WebServer::timer(int connfd, struct sockaddr_in client_address) {
    ...
    // 初始化client_data数据
    // 初始化定时器,设置回调函数和超时时间,绑定用户数据，将定时器添加到链表中
...
void WebServer::dealwithread(int sockfd) {
    ...
    if (m_actormodel == 1) {
        ...
        if (users[sockfd].write()) {
            LOG_INFO("read: send data to the client(%s)",
...
void WebServer::dealwithwrite(int sockfd){
    ...
    else
    {
        ...
        if (users[sockfd].write())
        {
            LOG_INFO("write: send data to the client(%s)", inet_ntoa(users[sockfd].get_address()->sin_addr));
