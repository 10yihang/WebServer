...
void http_conn::initmysql_result(connection_pool *connPool) {
    ...
    // 返回结果集中的列数
    int num_fieldss = mysql_num_fields(result);
...
// 将内核事件表注册读时间，ET模式，选择开启EPOLLONESHOT(缓冲区不为空就一直通知)
...
void http_conn::init(int sockfd, const sockaddr_in &addr, char *root,
    ...
    // 当浏览器出现连接重置时，可能是网站根目录出错或者http相应格式出错，或者访问的文件中内容完全为空
    doc_root = root;
    m_TRIGMode = TRIGMode;
...
// 循环读取客户数据，直到无数据连接或者关闭连接，因为非阻塞ET需要一次性将数据读完
    ...
    while ((m_check_state == CHECK_STATE_CONTENT && line_status == LINE_OK) ||
        ...
        switch (m_check_state) {
            case CHECK_STATE_REQUESTLINE: {
                ...
                // if(ret == BAD_REQUEST)
                // std::cerr<<"CHECK_STATE_REQUESTLINE\n";
                if (ret == BAD_REQUEST) return BAD_REQUEST;
                break;
            }
            case CHECK_STATE_HEADER: {
                ret = parse_headers(text);
                // if(ret == BAD_REQUEST) std::cerr<<"CHECK_STATE_HEADER\n";
                if (ret == BAD_REQUEST)
                    return BAD_REQUEST;
...
http_conn::HTTP_CODE http_conn::do_request() {
    ...
    if (cgi == 1 && (*(p + 1) == '2' || *(p + 1) == '3')) {
        ...
        if (*(p + 1) == '3') {
            ...
            strcat(sql_insert, "'");  // INSERT INTO user(username, passwd)
            ...
            if (users.find(name) == users.end()) {
                ...
                if (!res)
                    ...
                    strcpy(m_url, "/registerError.html");  // 数据库操作失败
            } else {
                strcpy(m_url, "/registerError.html");  // 已有该用户
            }
        } else if (*(p + 1) == '2') {
            // 登录则直接判断，若浏览器输入的用户名和密码可以直接查找到，返回1，否则返回0
    ...
    if (*(p + 1) == '0') {  // 进入注册界面
        ...
        std::cerr << m_real_file << '\n';

        free(m_url_real);
    } else if (*(p + 1) == '5') {  // 进入图片界面
        char *m_url_real = (char *)malloc(sizeof(char) * 200);
        strcpy(m_url_real, "/picture.html");
        strncpy(m_url_real + len, m_url_real, strlen(m_url_real));
    ...
    // std::cerr<<"m_file_address: "<<m_file_address<<'\n';
    close(fd);
    return FILE_REQUEST;
}
...
bool http_conn::write() {
    ...
    while (1) {
        ...
        std::cerr << "m_iv[0].len:" << m_iv[0].iov_len << ' '
                  << "base:" << (char *)m_iv[0].iov_base << '\n';
        std::cerr << "m_iv[1].len:" << m_iv[1].iov_len << ' '
                  << "base:" << (char *)m_iv[1].iov_base << '\n';
        ...
        bytes_to_send -= temp;
        if (bytes_have_send >=
            m_iv[0].iov_len) {  // 已发送完头部信息，ET边缘触发
            m_iv[0].iov_len = 0;
