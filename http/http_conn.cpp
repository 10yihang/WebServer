...
http_conn::HTTP_CODE http_conn::do_request(){
    ...
    if(cgi == 1 &&(*(p+1) == '2' || *(p+1) == '3')){
        ...
        if(*(p+1)=='3'){
            ...
            strcat(sql_insert, "'");//INSERT INTO user(username, passwd) VALUES('&name', '&password')
            ...
            if(users.find(name) == users.end()){
                ...
                else strcpy(m_url, "/registerError.html");//数据库操作失败
            }
            else {
...
bool http_conn::write(){
    ...
    while(1){
        ...
        bytes_to_send -= temp;
        if(bytes_have_send >= m_iv[0].iov_len){//已发送完头部信息，ET边缘触发
            m_iv[0].iov_len = 0;
